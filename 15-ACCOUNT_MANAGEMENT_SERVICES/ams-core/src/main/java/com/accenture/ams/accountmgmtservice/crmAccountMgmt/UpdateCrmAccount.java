...
import wsclient.commontypes.accountmgmt.avs.accenture.FlagTypeMF;
...
public class UpdateCrmAccount implements ActionExecutor {
	...
	public AccountMgmtResponse execute() throws MissingParameterException,
		...
		String crmAccountId = data.getCrmAccountId().toLowerCase();
		try {
			...
			LogUtil.writeCommonLog("INFO", UpdateCrmAccount.class, "WS",
					"I'm in UpdateCrmAccount.execute for crmAccountId: "
							+ crmAccountId + " - username : " + userName
							+ " - name : " + data.getName());
			LogUtil.writeCommonLog("INFO", UpdateCrmAccount.class, "WS",
					"UpdateCrmAccount payload : " + data.toString());
			if (currAccount == null)
	...
	private AccountUserBean updateFieldsAccAttribure(AccountUserBean aUser) {
		...
		if(data.getUserStatus() !=null) {
			LogUtil.writeCommonLog("INFO", UpdateCrmAccount.class, "WS","USER STATUS --------- " + data.getUserStatus());
			aUser.setStatusId(Long.valueOf(data.getUserStatus()));
		}
		return aUser;
	}
	...
	private List<AccountAttribute> createAccountAttributes(Long userId)
		...
		String blockPeriod = data
				.getCrmAccountPurchaseBlockingThresholdPeriod();
